...
function Transcoder(source) {
	...
	Transcoder.prototype._exec = function(a, ia) {
	...
	Transcoder.prototype._compileArguments = function (input) {
		var a = [],
				args = input ? this.inputArgs : this.args;
		for (var key in args) a = a.concat(args[key]);
		return a;
	};

	/* Add argument, if input is trusy: add it to input arguments */
	Transcoder.prototype._addArg = function (key, value, input) {
		var args = input ? this.inputArgs : this.args;
		args[key] = value;
	};
	
	Transcoder.prototype.exec = function() {
		return this._exec(this._compileArguments(), this._compileArguments(true));
	};
	...
	Transcoder.prototype.stream = function() {
		...
		a.push('pipe:1');
		return (this.stream = this._exec(a, this._compileArguments(true)).stdout);
	};
	...
	Transcoder.prototype.writeToFile = function(file) {
		var a = this._compileArguments();
		a = a.concat('-y', file);
		this._exec(a, this._compileArguments(true));
		return this;
	};

	Transcoder.prototype.flag = function(flag, input) {
		this._addArg('flag', [ '-fflags', flag ], input);
		return this;
	};
